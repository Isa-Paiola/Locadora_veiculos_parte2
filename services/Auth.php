...
class Auth{
    ...
    private function carregarUsuarios(): void {
        ...
        if(file_exists(ARQUIVO_USUARIOS)){
            // lê o conteúdo e dedodifica o JSON para o array
            ...
            $this -> usuarios = [
                ...
                [   'username' => 'usuario',
                    'password' => password_hash('usuario123', 
                    PASSWORD_DEFAULT),
                    'perfil' => 'usuario'
                ]
            ];
    ...
    // funcção para salvar usuários no arquivo JSON
    ...
    public function login(string $username, string $password): bool{
        ...
        foreach ($this ->usuarios as $usuario){
            if ($usuario['username'] === $username && password_verify($password, $usuario
            ['password'])){
                $_SESSION['auth'] = [
                    'logado' => true,
                    'username' => $username,
    ...
    public static function verificarLogin():bool{
        return isset($_SESSION['auth']) && $_SESSION 
        ['auth']['perfil'] === true;
    }
